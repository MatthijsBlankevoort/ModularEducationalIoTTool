...
void loop() {
  ...
  if (TinyWireS.available()) {          // got I2C input!
    ...
    TinyWireS.send(threshold);           //check
    TinyWireS.send(I2C_SLAVE_ADDR);     //ID
  }
  if (byteRcvd > threshold * 2) {                      //High value, high tone
    TinyTone(Note_C, 8, 500);
    delay(1000);
  }
  if (byteRcvd > threshold * 1.5) {                      //Lower value, lower tone
    TinyTone(Note_C, 4, 500);
    delay(1000);
  }
  if (byteRcvd > threshold) {                      //Lowest value, lowest tone
